...
const createTokens = exports.createTokens = (user, secret, secret2) => {
  const createToken = _jsonwebtoken2.default.sign({
    user: (0, _lodash.pick)(user, ['id', 'username'])
  }, secret, {
    expiresIn: '1h'
  });

  const refreshToken = _jsonwebtoken2.default.sign({
    user: (0, _lodash.pick)(user, 'id')
  }, secret2, {
    expiresIn: '7d'
  });
...
// along with the user.
const refreshTokens = exports.refreshTokens = (() => {
  var _ref = _asyncToGenerator(function* (token, refreshToken, models, SECRET, SECRET2) {
    let userId = -1;
    try {
    ...
    const [newToken, newRefreshToken] = yield createTokens(user, SECRET, refreshSecret);
    return {
      token: newToken,
  ...
  return function refreshTokens(_x, _x2, _x3, _x4, _x5) {
...
const tryLogin = exports.tryLogin = (() => {
  var _ref2 = _asyncToGenerator(function* (email, password, models, SECRET, SECRET2) {
    const user = yield models.User.findOne({ where: { email }, raw: true });
    if (!user) {
      return {
        ok: false,
    ...
    const valid = yield _bcrypt2.default.compare(password, user.password);
    if (!valid) {
      return {
        ok: false,
    ...
    const [token, refreshToken] = yield createTokens(user, SECRET, refreshTokenSecret);
  ...
  return function tryLogin(_x6, _x7, _x8, _x9, _x10) {
